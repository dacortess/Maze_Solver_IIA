...
int main(int argc, char **argv) {
    ...
    Node fin = Greedy(map, traverse, start, end, failure);
    auto endTime = std::chrono::high_resolution_clock::now();
    int totalTime = std::chrono::duration_cast<std::chrono::microseconds>(endTime - startTime).count();
    if(!failure){
        WriteSolutionPath(fin.path, "Greedy_path");
        WriteTraverse(traverse, "Greedy_traverse");
        WriteMemoryAndTime(totalTime, "Greedy_stats");
    }else{
        cout << "FAILED" << '\n';
    }
    return 0;
